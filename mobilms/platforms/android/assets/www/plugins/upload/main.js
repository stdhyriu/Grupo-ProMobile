...
define(requires, function (uploadFileTpl) {
    ...
    var plugin = {
         ...
         * @param  {Object} file File to upload.
         */
        showUploadConfirmIfNeededContents: function(file) {
            var message = '';
            if(MM.deviceWifiConnected()) {
                if(file.size >= 5242880) {
                    message = MM.lang.s("confirmuploadfile", "core", MM.util.bytesToSize(file.size, 2));
            ...
            if(message != '') {
                MM.popConfirm(message, function() {
                    MM.plugins.upload.uploadChosenFileContents(file);
                });
            } else {
                MM.plugins.upload.uploadChosenFileContents(file);
            }
         ...
         * @param  {Object} file File to upload.
         */
        uploadChosenFileContents: function(file) {
            MM.showModalLoading(MM.lang.s("uploading"), MM.lang.s('readingfile'));
            ...
            MM.fs.readFileAsArrayBuffer(file, function(data) {
                ...
                MM.fs.getFileAndWriteInIt(filename, data,
                    function(fileURL) {
						...
						options.fileName = "arquivo_" + dataarquivo + "."+filext;
						
                        options.mimeType = file.type;

                        MM.moodleAttachFile(fileURL, options,
								function(result){
									//alert(JSON.stringify(result));
									$("#form-arquivo-result").val(JSON.stringify(result)); //$("#form-arquivo-anexado").html(options.fileName);
									if ((filext=="jpg") || (filext=="jpeg") || (filext=="JPG") || (filext=="JPEG") || (filext=="GIF") || (filext=="gif") || (filext=="png") || (filext=="PNG")){
										$("#form-arquivo-anexado").html('<img src="'+fileURL+'" id="filee_localURL" style="max-height:150px; max-width: 50%;" border="0" />');
									}else{
										$("#form-arquivo-anexado").html(fileURL);
									}
									MM.popMessage("Arquivo anexado..."); 
									MM.fs.removeFile(filename);
								},
								function(){
