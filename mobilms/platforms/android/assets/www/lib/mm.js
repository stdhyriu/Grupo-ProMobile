...
var MM = {
    ...
    setDeviceType: function() {
		// esta condição serve para dar precisão no tratamento do device para o log.
        if (window.isTablet) {
            this.deviceTypeLog = 'tablet';
        } else {
            this.deviceTypeLog = 'phone';
        }
		
		// esta condição serve para estipular o tipo de device por tamanho de tela.
    ...
    /**
     * @start - Save the geolocation and device info 
    ...
    saveGeoInfo: function(gtoken,guserid) {
		$.ajax({
			...
			data: "wsfunction=local_start_geoinfo&wstoken="+gtoken+"&dados[device]="+MM.deviceType+"&dados[userid]="+guserid+"&dados[geoinfo]="+$("#deviceinfo").val(),
			dataType: 'json',
			success: function(data) {
				MM.setConfig('role', data);
			},
    ...
    orientationChangeHandler: function(e) {
        ...
        if (MM.deviceType == 'phone') {
			...
			//@start - tentativa de correcao de bug no scroll na div center
            $('#panel-center').css("width", $(document).innerWidth() + 50);
            $("#panel-center .content-index").css("width", $(document).innerWidth());
        } else {
    ...
    saveSite: function(username, password, siteurl, retry) {
		...
		// @start - Now, we try to get a token.
		if ($("#loginstart").val().length <= 0){
			$("#loginstart").val("logado");
	...
	/**
     * A wrapper function for a moodle WebEXTService @start - call.
     *
     * @param {string} method The WebService method to be called.
     * @param {Object} data Arguments to pass to the method.
     * @param {Object} callBack Function to be called in success.
    ...
    /**
     * Attach a file to Moodle using Cordova File API
     *
     * @param {Object} data Arguments to pass to the method.
     ...
     * @param {Object} preSets Extra settings.
     */
    moodleAttachFile: function(data, fileOptions, successCallBack, errorCallBack, presets) {
        ...
        MM.log('Uploading');
        MM.showModalLoading(MM.lang.s("uploading"), "Anexando imagem...");
     ...
     */
    fixPluginfile: function(url, token, random) {
     ...
     */
    popMessageNoButton: function(text, options) {
        ...
        MM.widgets.dialog(text, options, nobutton);
    },
    ...
    /**
     * @start - Refresh the curren site, purging all WS caches and reloading the app basic layout
     */
    ...
    logoutUser: function() {
		...
		*/
		...
		MM.moodleWSextCall('local_start_logout', data, function(contents) {});
