...
MM.util = {
    ...
    getMoodleFilePath: function (fileurl, courseId, siteId, token) {
		...
		if (courseId == 1) {
			var downloadURL = MM.fixPluginfile(fileurl, token, 1);
        }else{
	...
	// @start - conversao timestamp
    dtMomentTSDate: function(dtdate) {
        var rdtdate = moment.unix(dtdate).add('25','m').format('DD/MM/YYYY');
        return rdtdate;
    },

    dtMomentTShour: function(dthour) {
        var rdthour = moment.unix(dthour).format('HH:mm');
        return rdthour;
    }
};
